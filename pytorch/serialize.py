...
class ModelSerializer:
    ...
    def __init__(self, project_path, save_dir=".saved", model_folder_prefix="model_"):
        ...
        self.project_save_path = os.path.join(self.project_path, "saved")
    ...
    def get_latest_saved_model_number(self):
        numbers = set(
            map(self.get_model_folder_number,
                filter(self.is_model_folder_like_name,
                filter(self.is_model_folder_like_name,
                       map(attrgetter("name"),
    ...
    def get_dump_paths(self, replace_last=False):
        number = self.get_latest_saved_model_number() + (0 if replace_last else 1)
