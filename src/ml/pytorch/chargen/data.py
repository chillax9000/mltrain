    ...
    def build_category_lines(self):
        ...
        for lang in wiki.Lang:
            ...
            for page in wiki.default_pages[lang]:
                ...
                words_ = ml.util.text.get_words_from_text(text)
                words_ = map(str.lower, map(ml.util.text.unicode_to_ascii, filter(str.isalpha, words_)))
...
class DataSentence(Data):
    ...
    def build_category_lines(self):
        ...
        for lang in wiki.Lang:
            ...
            for page in wiki.default_pages[lang]:
                text = wiki.get_cleaned_text(page, lang)
                sentences_ = list(ml.util.text.get_sentences_from_text(text))
                n_imported = len(sentences_)
                sentences_ = list(filter(self.acceptable_input,
                                         map(str.lower, map(ml.util.text.unicode_to_ascii, sentences_))))
